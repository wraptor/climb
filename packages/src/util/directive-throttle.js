...
export default {
  mounted(el, binding) {
    ...
    if (valueType === '[object Function]') {
      ...
      if (Object.prototype.toString.call(binding.value[0]) === '[object Function]') {
        el.addEventListener(
          'click',
          throttle(() => {
        ...
        console.error('v-click指令传惨错误,数组首个参数必须为function类型', binding.value)
      }
    } else {
      console.error('v-click指令传惨错误,只支持function或array类型', binding.value)
    }
  }
}
