  ...
  methods: {
    initDefaultForm() {
      for (let i = 0; i < this.myOption.columns.length; i++) {
        ...
        if (item.value !== undefined && this.backForm[item.prop] === undefined) {
    ...
    disabledFilter(item) {
      if (this.type === undefined) {
        if (Object.prototype.toString.call(this.myOption.disabled) === '[object Function]') {
          return this.myOption.disabled(this.form, this.backForm)
        }
        return this.disabled || this.myOption.disabled
      }
      if (Object.prototype.toString.call(item[`${this.type}Disabled`]) === '[object Function]') {
